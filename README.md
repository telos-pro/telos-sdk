...
任何 STELA 入口（proxy / SDK transport）都会把每次 call 的 normalized usage
按行追加到 `usage_log` jsonl。`stela dashboard` 把它聚合成单文件 HTML——
...
# 多个 log / glob 一起来
stela dashboard --usage-log '/tmp/runs/*.usage.jsonl' \
                --usage-log ~/.stela/usage.jsonl
```

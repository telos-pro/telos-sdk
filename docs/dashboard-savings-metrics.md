...
入口：`GET /__stela/dashboard`（proxy 内嵌）或 CLI:
...
> **看板视角**：面向"用户/老板"——回答"接入 STELA 之后省了多少 token、省了多少钱"。
...
- `若关 STELA 预计要付 $A · 实付 $B · 节省 X%` —— 反事实 vs 实付 vs 占比

切到"without STELA · 反事实"视角时，hero 变成：
- **prompt tokens (no cache)** = `raw_input + cache_read + cache_write`（全部按 input 价）
- **cost (no STELA)** = §1.4 的反事实成本
...
"with STELA"视角下显示真实四色分布。
"without STELA · 反事实"视角下，前三个塌成一个橙色块（所有 prompt token 都按 input 价计）。
...
| 模式 | 看什么 |
|---|---|
| **实际（开 STELA）** | 真实四色 token mix、命中率、实际 saved $ |
| **反事实（不开 STELA）** | 把所有 prompt token 当 raw_input、按 input 价估算总成本 |
| **并排对比** | 左侧反事实大数字、右侧实际大数字、底下一行"净节省 $X · X% off" |

选择会通过 `localStorage.stela.dashboard.mode` 持久化。
...
最干净的对照基线。STELA 的价值就是缩小 (counterfactual − actual) 这个差。

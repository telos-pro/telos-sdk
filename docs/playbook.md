# STELA Playbook —— 从理念到最佳实践

> 用户侧操作手册。读完你能回答三个问题：STELA 为什么存在、怎么把它接进
> 你的 agent、接进去之后怎么用好它。
>
> - 想要逐项 CLI 参考 → [User-guide.md](User-guide.md)
> - 想懂代码架构 → [ARCHITECTURE.md](ARCHITECTURE.md)
> - 想懂协议规范 → [2026-05-06-stela-protocol.md](2026-05-06-stela-protocol.md)
>
> 最后更新：2026-05-16
...
STELA 不是一个"更聪明的 prompt 框架"。它只做一件事：识别请求里哪些是
...
STELA 稳的是**请求前缀**。但 agent 每轮还会往对话尾巴追加大段工具输出
（bash / pytest / docker 日志，动辄几千 token）。这部分 STELA 管不到。

所以有第二条线 —— **RTK 输出过滤**（吸收 [rtk-ai/rtk](https://github.com/rtk-ai/rtk)
的思路）：在请求进 STELA 之前，把 `tool_result` 里的大段重复输出压掉。

两条线互相独立，由一个四态开关控制：

| 开关 | STELA 前缀缓存 | RTK 工具过滤 |
|---|:---:|:---:|
| `none` | ✗ | ✗ |
| `stela` | ✓ | ✗ |
| `rtk` | ✗ | ✓ |
...
> 不开 STELA：工具输出缩了，但稳定前缀仍每轮重算。两条线合起来收益最大。
...
```bash
python -c "import stela; print(stela.__file__)"   # .../telos-sdk/__init__.py
stela --help                                       # proxy / init / dashboard / replay
```
...
两条路径**功能等价**（同一 STELA 管线、同一状态累积），区别只在进程
...
```bash
# ① 起代理（默认 mode=stela，默认录会话到 ~/.stela/corpus）
...
```bash
stela init --agent claude-code --uninstall   # 精确还原 install 前状态
...
```bash
stela init --agent generic    # 打印 export 指令，自己加到 shell rc / Dockerfile / k8s env
...
把 `anthropic.Anthropic()` 换成 `StelaAnthropicTransport`，`.messages.create()`
调用一字不改：
...
OpenAI 形状的 agent 用 `StelaOpenAITransport`（`.chat.completions.create`）。
...
```bash
# 进程级默认
stela proxy --mode both

# 单条请求覆盖（首请求的取值会 sticky 到该 session）
curl ... -H 'X-Stela-Mode: rtk'
```

四态：`none` / `stela` / `rtk` / `both`，含义见 §1.5。

> **默认建议**：先用 `stela`（稳妥，不改工具结果字节），观察一段时间确认
> 无异常，再切 `both`。RTK 会改写 `tool_result`，先验证你的 agent 不依赖
> 工具输出的逐字节原文。

### 5.2 对比实验 —— 哪种优化值得开？

STELA 提供两种受控对照，回答"开 STELA / RTK 到底省多少"。
...
```bash
stela replay --list                              # 看语料库里有哪些会话
stela replay --session <id>                       # 默认 4 mode 全跑
stela dashboard --usage-log ~/.stela/usage.jsonl  # A/B 对比面板看结果
```
...
起两个独立 agent 会话、用户输入相同，各带不同 `X-Stela-Mode` + 相同
`X-Stela-Compare-Group`，dashboard 同一面板并排。能测端到端任务成本，
...
1. **同一对话用同一 `session_id`**。多轮 cache 累积全靠它。路径 A 传
   ...
   `x-stela-session` header。
2. **先 `stela` 后 `both`**。先验证 STELA 前缀缓存稳定无异常，再叠加会
   改写工具结果的 RTK。
3. **接入后第一件事是看 dashboard**。`/__stela/dashboard` 或
   `stela dashboard`，确认 `cache_read` 在涨、`cache hit%` 合理。
...
7. **生产用非 strict（默认）**。STELA 失败自动降级 passthrough，正确性
...
| 别这样 | 为什么 | 改成 |
|---|---|---|
| SDK transport 路径用 `stream=True` | 流式没接 STELA 处理，直接透传 | 路径 A 用非流式；要流式走路径 B（代理完整支持 SSE）|
...
| 看板 | 入口 | 看什么 |
|---|---|---|
| 省钱看板 | `/__stela/dashboard` 或 `stela dashboard` | 省了多少 token / 美刀、A/B 对比、mode breakdown |
| 开发者页面 | `/__stela/developer` | 当前内存里每个 session 的 IR 结构、PIN/FOLD/DROP 分布、工具统计 |
| usage_log | `~/.stela/usage.jsonl` | 逐调用的原始数据 |
...
# 有没有降级到 passthrough（STELA 出错的信号）
...
| 看到 `passthrough` 记录 | STELA 管线抛异常、自动降级 | 看代理日志首次 traceback；dev 阶段加 `--strict` 让它显式爆 |
| `StelaInvariantError: Band order violated` | harness 输出违反 §5 | 这是 STELA-side bug；扩展新 harness 时 message 末尾过一遍 `enforce_band_order` |
...
2. `pip install -e .`，`stela proxy` 起代理，`stela init --agent claude-code`。
3. 正常用几天 agent，让语料库自然积累。
4. `stela dashboard` 看省钱看板，确认 cache 在命中。
5. `stela replay --session <id>` 对一个真实会话做 4-mode 对照，看 A/B 面板
   决定要不要全量切 `both`。

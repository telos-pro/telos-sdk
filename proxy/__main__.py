...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="telos.proxy",
        description="TELOS Anthropic 反向代理。"
    ...
    parser.add_argument("--mode", default="telos", choices=list(MODE_LABELS),
                        help="默认优化开关：none=纯透传 / telos=只前缀缓存 / "
                             ...
                             "单条请求可用 X-Telos-Mode header 覆盖。")
    parser.add_argument("--strict", action="store_true",
                        help="TELOS 失败时返回 500（默认降级到 passthrough）")
    parser.add_argument("--corpus-dir", type=Path, default=DEFAULT_CORPUS_DIR,
                        help=f"会话语料目录，录原始请求供 telos replay 重放"
    ...
    parser.add_argument("--dashboard-refresh", type=int, default=5,
                        metavar="SECONDS",
                        help="GET /__telos/dashboard 的 meta-refresh 间隔，"
